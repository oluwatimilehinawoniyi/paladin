<script lang="ts">
	...
	$: interviewRate = totalApplications > 0 ? ((totalInterviews / totalApplications) * 100).toFixed(1) : '0.0';
	$: offerRate = totalApplications > 0 ? ((totalOffers / totalApplications) * 100).toFixed(1) : '0.0';
	$: conversionRate = totalInterviews > 0 ? ((totalOffers / totalInterviews) * 100).toFixed(1) : '0.0';
...
<section class="flex h-screen w-full flex-col gap-8 overflow-hidden p-4 md:pt-14">
	...
	<div class="flex w-full flex-col justify-between gap-4 sm:flex-row sm:items-end">
		...
		<div class="flex items-center gap-2">
			<button class="flex items-center gap-2 rounded-md border border-gray-300 px-3 py-2 text-sm text-gray-600 hover:bg-gray-50">
				...
				<div class="flex items-center justify-between">
					<div>
						...
						<div class="mt-2 w-full bg-gray-200 rounded-full h-2">
							<div class="bg-yellow-500 h-2 rounded-full" style="width: {interviewRate}%"></div>
						</div>
					</div>
			...
			<div class="rounded-lg border border-gray-200 bg-white p-6">
				<div class="flex items-center justify-between">
					<div>
						...
						<div class="mt-2 w-full bg-gray-200 rounded-full h-2">
							<div class="bg-green-500 h-2 rounded-full" style="width: {offerRate}%"></div>
						</div>
					</div>
			...
			<div class="rounded-lg border border-gray-200 bg-white p-6">
				<div class="flex items-center justify-between">
					<div>
						...
						<p class="text-sm text-gray-500 mt-1">Interviews to offers</p>
					</div>
				...
				<div class="rounded-lg border border-gray-200 bg-white p-6">
					<div class="mb-6 flex items-center justify-between">
						<div>
							...
							<p class="text-sm text-gray-600">Monthly breakdown of applications, interviews, and offers</p>
						</div>
					</div>
			...
			<div class="space-y-6">
				...
				<div class="rounded-lg border border-gray-200 bg-white p-6">
					<div class="mb-4">
						...
						<p class="text-sm text-gray-600">Which profiles are used most frequently for applications</p>
					</div>
					...
					<div class="space-y-4">
						{#each insights as insight}
							...
							<div class="flex gap-3 rounded-md bg-gray-50 p-3">
								<div class="flex h-8 w-8 items-center justify-center rounded-full bg-{insight.type === 'success' ? 'green' : insight.type === 'warning' ? 'yellow' : 'blue'}-100">
									<IconComponent class="h-4 w-4 text-{insight.type === 'success' ? 'green' : insight.type === 'warning' ? 'yellow' : 'blue'}-600" />
								</div>
								<div class="flex-1">
			...
			<div class="grid grid-cols-1 gap-6 md:grid-cols-3">
				...
				<div class="space-y-3">
					...
					<div class="w-full bg-gray-200 rounded-full h-3">
						<div 
							class="bg-[#ff4d00] h-3 rounded-full transition-all duration-500" 
							style="width: {applicationProgress}%"
						></div>
					</div>
					<p class="text-xs text-gray-500">
						{applicationGoal - currentApplications > 0 ? `${applicationGoal - currentApplications} more to reach goal` : 'Goal achieved! 🎉'}
					</p>
				</div>
				...
				<div class="space-y-3">
					...
					<p class="text-xs text-gray-500">
						{interviewGoal - currentInterviews > 0 ? `${interviewGoal - currentInterviews} more to reach goal` : 'Goal exceeded! 🎉'}
					</p>
				</div>
				...
				<div class="space-y-3">
					...
					<p class="text-xs text-gray-500">
						{offerGoal - currentOffers > 0 ? `${offerGoal - currentOffers} more to reach goal` : 'Goal exceeded! 🎉'}
					</p>
				</div>
			</div>
		</div>
		...
		<div class="rounded-lg border border-gray-200 bg-white p-6">
			...
			<div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center">
				...
				<div class="relative">
					...
					<ChevronDown class="absolute right-2 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
				</div>
				...
				<table class="w-full">
					<thead>
						<tr class="border-b border-gray-200">
							...
							<th class="text-left py-3 px-4 font-medium text-gray-600">Date Applied</th>
							<th class="text-left py-3 px-4 font-medium text-gray-600">Profile Used</th>
					...
					<tbody>
						{#each filteredApplications as application (application.id)}
							...
							<tr class="border-b border-gray-100 hover:bg-gray-50">
								...
								<td class="py-4 px-4 text-gray-600">{application.position}</td>
								<td class="py-4 px-4 text-gray-600">{application.dateApplied}</td>
								<td class="py-4 px-4 text-gray-600">{application.profileUsed}</td>
								<td class="py-4 px-4">
									<span class="inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-medium {getStatusColor(application.status)}">
			...
			<div class="mt-6 flex items-center justify-between border-t border-gray-200 pt-4">
				...
				<div class="flex items-center gap-4 text-sm">
					<div class="flex items-center gap-1">
						...
						<span class="text-gray-600">Applied ({applications.filter(a => a.status === 'Applied').length})</span>
					</div>
					<div class="flex items-center gap-1">
						<div class="h-2 w-2 rounded-full bg-yellow-500"></div>
						<span class="text-gray-600">Interview ({applications.filter(a => a.status === 'Interview').length})</span>
					</div>
					<div class="flex items-center gap-1">
						<div class="h-2 w-2 rounded-full bg-green-500"></div>
						<span class="text-gray-600">Offer ({applications.filter(a => a.status === 'Offer').length})</span>
					</div>
					<div class="flex items-center gap-1">
						<div class="h-2 w-2 rounded-full bg-red-500"></div>
						<span class="text-gray-600">Rejected ({applications.filter(a => a.status === 'Rejected').length})</span>
					</div>
				</div>
			</div>
		</div>
